									...
									<tr>
										<td align="center" valign="top" class="textContent">
											<h1 style="color:#FFFFFF;line-height:100%;font-family:Helvetica,Arial,sans-serif;font-size:35px;font-weight:normal;margin-bottom:5px;text-align:center;">
												{{ $order->name }}<br />
												<a target="_blank" href="https://courier.pathao.com/deliveries/{!! $data['consignment_id'] !!}/edit">{{ $data['status'] }}</a>,
											</h1>
											<h2>
												Dispatched {{ $order->dispatch_date->diffForHumans() }} ({{ $order->dispatch_date->format('M-d') }})
											</h2>
										</td>
									</tr>
								</table>
								...
								<table border="0" cellpadding="0" cellspacing="0" width="100%" class="emailButton" style="background-color: white;">
									<tr>
										<td align="center" valign="middle" class="buttonContent" style="padding-top:15px;padding-bottom:15px;padding-right:15px;padding-left:15px;color:#666666; text-align:left; font-size: 14px;">
											<h4>Courier details are as follows: </h4>
											<div class="box">
												@foreach( $data as $k=>$v )
													@if( gettype($v) != 'array' )
														<p><span style="width: 100px; padding: 5px; background-color: #efefef;">{{ $k }}</span> {{ $v }}</p>
													@else
														<b>{{ $k }}</b>
														@foreach( $v as $ind => $val )
															<p><span style="width: 100px; padding: 5px; background-color: #efefef;">{{ $ind }}</span> {{ $val }}</p>
														@endforeach
													@endif
												@endforeach
											</div>
